...
class Voronoi:
    ...
    def setup(self, height: int, width: int, seeds: Array):
            ...
            with jax.default_device(jax.devices("cpu")[0]):
                # Use sentinel for "no seed"
                sentinel = jnp.array([-1, -1], dtype=self.seeds.dtype)
                arr = jnp.full((height, width, 2, 2), sentinel, dtype=self.seeds.dtype)

                seed_values = jnp.tile(self.seeds[:, None, :], (1, 2, 1))
                self.seed_map = arr.at[self.seeds[:, 0], self.seeds[:, 1]].set(seed_values)

                max_dim = max(height, width)
                max_exp = int(jnp.floor(jnp.log2(max_dim // 2)))
                base = max_dim // 2
    ...
    def _jfa_step(arr: Array, offset: int):
        ...
        sentinel = jnp.array([-1, -1], dtype=arr.dtype)
        ...
        def step(i, j):
            ...
            info = arr[candidates_y, candidates_x]    # shape (9, 2, 2)
            all_sites = info.reshape(-1, 2)           # (18, 2)

            # Mask out sentinel / empty sites
            valid = ~jnp.all(all_sites == sentinel, axis=-1)
            # If nothing valid, just keep current value (or pos as its own seed)
            def no_valid():
                return arr[i, j]

            def some_valid():
                valid_sites = jnp.where(valid[:, None], all_sites, sentinel)
                # Distances only for valid sites, inf otherwise
                dists = jnp.where(
                    valid, jnp.linalg.norm(valid_sites - pos, axis=-1), jnp.inf
                )
                idxs = jnp.argsort(dists)
                first = valid_sites[idxs[0]]

                # Second closest among remaining valid
                mask2 = valid & ~jnp.all(valid_sites == first, axis=1)
                dists_masked = jnp.where(mask2, dists, jnp.inf)
                idx2 = jnp.argmin(dists_masked)
                second = valid_sites[idx2]
                return jnp.stack([first, second], axis=0)

            return jax.lax.cond(jnp.any(valid), some_valid, no_valid)

        vmapped = jax.vmap(jax.vmap(step, in_axes=(None, 0)), in_axes=(0, None))
        grid_y = jnp.arange(H)
        grid_x = jnp.arange(W)
        return vmapped(grid_y, grid_x)
    ...
    def get_index_map(jfa_map: Array, seeds: Array):
        ...
        sentinel = jnp.array([-1, -1], dtype=flat_coords.dtype)
        valid = ~jnp.all(flat_coords == sentinel, axis=-1)
        ...
        indices = jnp.where(valid & any_match, indices, -1)
